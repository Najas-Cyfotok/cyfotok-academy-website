    ...
    <footer className="p-5">
      ...
      <div className="my-6 grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        <div>
          ...
          <hr className="my-3 " />
          {FooterResources.map((company, index) => (
            <ListItems title={company.title} url={company.link} key={index} />
          ))}
        </div>
        <div>
          ...
          <hr className="my-3 " />
          {FooterSupport.map((company, index) => (
            <ListItems title={company.title} url={company.link} key={index} />
          ))}
        </div>
      </div>
      <p className="text-black/80 text-center font-semibold">Cyfotok Academy,© All Rights Reserved 2024</p>
      <hr className="my-3"/>
    </footer>
  );
};
